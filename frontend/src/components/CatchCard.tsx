    ...
    <Link to={`/catches/${c.id}`} className="block group">
      <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden hover:shadow-md transition-shadow">
        {c.photo_url ? (
          ...
          <div className="w-full h-44 bg-gradient-to-br from-blue-100 to-slate-100 dark:from-slate-700 dark:to-slate-600 flex items-center justify-center text-4xl">
            🎣
          </div>
        )}
        ...
        <div className="p-4">
          <div className="flex items-start justify-between gap-2">
            <h3 className="font-semibold text-slate-800 dark:text-slate-100 text-lg leading-tight">{c.species}</h3>
            {c.kept && (
              <span className="shrink-0 text-xs bg-amber-100 dark:bg-amber-900/40 text-amber-700 dark:text-amber-400 px-2 py-0.5 rounded-full font-medium">
                Kept
              </span>
            )}
          </div>

          <div className="mt-1.5 flex flex-wrap gap-x-3 gap-y-1 text-sm text-slate-500 dark:text-slate-400">
